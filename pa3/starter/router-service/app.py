...
def route_order(ch, method, properties, body):
    ...
    # Declare every output queue up front, idempotently.
    ...
    for item_index, item in enumerate(items):
        ...
        if routing_key is None:
            # Unknown type: don't let it silently vanish. Log it and fall
            # back to the physical queue, since that is the safest default
            # (a human eventually looks at a physical shipment; a dropped
            # digital/subscription item just disappears). This choice is
            # explained in the ADR.
            print(f"[Router] WARNING: unknown item type '{item_type}' "
                  f"for order {order_id}, item {item_index} -- "
                  f"falling back to orders.physical")
            routing_key = ROUTES['physical']
